import { ChangeEvent, FormEvent } from 'react'; import { AuthFormData } from '../types/AuthFormInterfaces';
...
export const AuthForm: React.FC<AuthFormData> = ({ title, buttonText, data, handleChange, handleSubmit, submitFunc, onSuccess }) => {
    const internalHandleSubmit = async (e: React.FormEvent<HTMLFormElement>) => { 
        e.preventDefault();
        if (submitFunc) { try { await submitFunc(data); if(onSuccess) onSuccess(); } catch (error) { console.error(error); } } else { handleSubmit(e); }
     }
        return <div className="flex justify-center py-24 3xl:py-56"><div className="border rounded-md p-4 sm:p-14">
