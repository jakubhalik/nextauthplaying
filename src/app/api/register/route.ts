...
export async function POST(request: NextRequest) { 
    const body = await request.json(); const { name, email, password } = body; console.log(body); 
    if(!name || !email || !password) { return new NextResponse('Missing name, email, or password', { status: 400 }); }
    const exists = await prisma.user.findUnique({ where: { email: email } }); if(exists) { return new NextResponse('User already exists', { status: 400 }); }
